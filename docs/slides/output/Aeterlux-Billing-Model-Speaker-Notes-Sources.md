# Aeterlux 收費模式與成本來源｜講稿及計算備註

更新日期：2026-09-15。幣別均為新臺幣，GiB 為二進位容量單位。這是試營運商業模式提案，非已上線的付費功能或確定售價。頁面區間是預算情境，非廠商保價。以下精確數字用來說明推算方法，仍包含待驗證假設。

## 數字的來源口徑

[1] 本地設備：以[欣亞整機售價](https://www.sinya.com.tw/prod/214584)為查價依據，加上自訂周邊預算。25–30萬元不是通路直接報出的套裝區間。
[2] Arweave：使用[Turbo 即時費率 API](https://payment.ardrive.io/v1/rates)和[單 GiB 上傳報價](https://payment.ardrive.io/v1/price/bytes/1073741824)換算，依[官方 SDK 說明](https://docs.ar.io/sdks/turbo-sdk/turbounauthenticatedclient)核對單位。本文採2026-09-15 20:57的快照；點開API會看到當下的新數字，可能不同。
[3] 電費：參考[台電115年上半年電價費率](https://hc1.taipower.com.tw/2289/2558/49405/49408/67252/)的官方制度資料；5–7元／度及機器功耗是本專案預算情境，不是台電保證費率。
...
金流來源：[綠界一般賣家服務費率](https://www.ecpay.com.tw/Business/payment_fees)，國內信用卡2.75%未稅、手續費另加5%稅及每筆1元處理費；本模型採3%＋1元作預留。正式定期扣款與特約費仍依合約，不能套用一般賣家頁面當完整訂閱金流報價。
...
欣亞 MSI MEG Vision X AI 2NVZ9-016TW，Ultra 9-285K／RTX5090 32G／DDR5 64G／2TB SSD，查價 [NT$249,800（欣亞商品頁）](https://www.sinya.com.tw/prod/214584)，頁面為貨到通知。
查價數字來自[整機商品頁](https://www.sinya.com.tw/prod/214584)；UPS、備份碟與網路設備金額是內部採購預算，未取得報價。
整機249,800＋UPS預留10,000＋備份碟6,000＋網路設備4,000＝NT$269,800。整機已含 GPU、CPU、RAM、電源和散熱，不再重複加購顯卡成本。
頁面區間25–30萬元是採購預算帶，中心值269,800。36個月直線攤提、殘值0：269,800／36＝NT$7,494.44／月。對應區間250,000／36至300,000／36＝6,944.44–8,333.33／月。
購入款為一次現金支出；採折舊估每月成本時不再將整筆購入款當月費成本。未含第二台備援、辦公室及冷氣裝機。

## 五、電費與散熱：頁面約 NT$1,500–6,000／月／台

制度參考：[台電電價費率及說明](https://hc1.taipower.com.tw/2289/2558/49405/49408/67252/)。以下功耗、運作時數、忙碌占比及冷卻係數均為內部估算，需電表及帳單確認。
...
本次API報價時間：2026-09-15T20:57:04.7959249+08:00。
1 GiB＝1,073,741,824 bytes。
[GET /v1/rates 即時費率](https://payment.ardrive.io/v1/rates)：每GiB WINC=13980480526047、USD=57.857680946256、perDataItemFeeWinc=7434945。
[GET /v1/price/bytes/1073741824 單 GiB 報價](https://payment.ardrive.io/v1/price/bytes/1073741824)：WINC=13980487960992。
換算依據：[Turbo SDK getFiatRates／getFiatEstimateForBytes](https://docs.ar.io/sdks/turbo-sdk/turbounauthenticatedclient)。32元匯率、5元差額、20%緩衝與30–35元匯率情境是規劃假設，非API報價。
美元成本＝bytes報價WINC／rates.winc×rates.fiat.usd＝US$57.85771172／GiB。WINC以整數精度處理為正式實作要求，本估算此級距仍在JavaScript安全整數範圍。
規劃匯率32元／美元，原始成本＝57.85771172×32＝NT$1,851.45／GiB。
另預留每GiB 5元小檔管理差額，價格／匯率／重試緩衝20%：ceil[(1,851.45＋5)×1.20]＝NT$2228／GiB。
單檔bytes報價已含該data item費用，不重複加計。正式訂單需按加密與封裝後各data item逐筆估價，分片及大量小檔可能改變結果。
頁面區間以本次美元報價、匯率30–35及最多20%緩衝作情境：原始成本約1,735.73–2,025.02元；高情境(美元成本×35＋5)×1.20約2,436.02元，取概略展示帶1,800–2,600元。它不是報價承諾或價格統計信賴區間。
...
【內部預算，非外部報價】本節每一項金額均由團隊先行編列，目前沒有可以證明這些金額的廠商報價連結。正式簽約後應以實際方案頁／報價單替換。
...
本節依據：[專案本機部署與效能紀錄](../../server/SERVER.md)，IndexTTS2既有RTF約2.7。3.5 GPU分鐘／音訊分鐘、每次回覆2秒與辨識預算0.50元／分鐘為內部假設。
...
文字訂閱滿額GPU時間＝300×2／3600＝0.17小時；電費0.75元。
聲影訂閱滿額GPU時間＝600×2／3600＋60×3.5／60＝3.83小時；電費17.25元。
輸入語音辨識另留每分鐘0.50元，60分鐘為30元，這是預算非指定供應商報價。若改本地辨識，移除此API預留並加回GPU時間。
文字戶每月變動成本：電費0.75＋客服／處理20＋金流9.97＝NT$30.72。不含稅收入扣變動成本＝299／1.05−30.72＝NT$254.04。
聲影戶每月變動成本：電費17.25＋辨識30＋客服／處理40＋金流27.97＝NT$115.22。貢獻餘額＝899／1.05−115.22＝NT$740.97。
...
本節Arweave成本承接第六節[Turbo API](https://payment.ardrive.io/v1/rates)快照計算，金流參考[綠界費率](https://www.ecpay.com.tw/Business/payment_fees)並保守預留；其他人工、gas、校驗及售後金額都是預算。售價由內部提案設定。

首1GiB服務售價4,990元：Arweave成本預算2228＋人工整理1.5小時×400＝600＋鏈上交易預留150＋校驗交付100＋金流150.70＋重做／售後準備200＝NT$3,428.70。貢獻餘額＝4990／1.05−3,428.70＝NT$1,323.68。
新增1GiB售價3,490元：2228＋處理100＋校驗50＋金流105.70＝NT$2,483.70，貢獻餘額＝3490／1.05−2,483.70＝NT$840.11。
...
月固定成本＝設備折舊7,494.44＋待機電費657＋平台維運7,500＝NT$15,651.44。
假設100個付費塔位，文字60戶、聲影40戶，全部用滿：含稅月收60×299＋40×899＝53,900；不含稅收入51,333.33；變動成本60×30.72＋40×115.22＝6,452.00；扣固定後餘額＝NT$29,229.89。
此餘額未扣全職人事、研發、獲客、租金及所得稅，不是淨利，也未把一次性封存收入當訂閱收入。
60／40組合平均單戶貢獻＝NT$448.81，覆蓋本表基本成本約需ceil(15,651.44／448.81)＝35戶。加入每月100,000元人事開發預算，紙上需258戶，但尚受單機容量限制。
100戶GPU月忙碌時數＝60×0.166667＋40×3.833333＝163.33小時。規劃可出售容量730×40%＝292小時，預留尖峰、維護與建立分身。此時同組合滿額理論約178戶，不能推成可同時在線數或延遲保證。更換TTS後必須重新估算。
...
電價制度參考：[台電官方費率及說明](https://hc1.taipower.com.tw/2289/2558/49405/49408/67252/)（正式估價依實際用戶帳單，本計算5–7元是預算）
本機部署與IndexTTS2既有記錄：docs/server/SERVER.md，第11、12節。
Chatterbox V3目前仍待本機測試：https://github.com/resemble-ai/chatterbox
金流公開費率已核對：[綠界服務費率表](https://www.ecpay.com.tw/Business/payment_fees)，但仍未取得專案正式支付合約；RPC、Lit與雲端費用亦未取得正式合約，相關數字仍為預留。
