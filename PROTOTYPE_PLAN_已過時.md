...
> 本文件是 [idea_已過時.md](idea_已過時.md) 的工程落地計畫,將「數位塔位 / 主權數位先祖系統」的三層架構切成可實作的 prototype,涵蓋:**身分層(智能合約)、儲存層(Arweave/Irys)、運算層(AI 推理)、後端服務、前端介面、線下訓練腳本**。
...
**身分層為何「真的」上鏈?**
專案核心論述是「鏈上家譜 + NFT 持有 = 數據主權」,用 DB 模擬會直接打臉 idea_已過時.md 的價值主張。Sepolia / Base Sepolia testnet 完全免費(水龍頭領 ETH),沒有理由不做真合約。
...
- 持有者可呼叫 `safeMintWithParent` 為自己持有的塔位開子節點(對應 idea_已過時.md §3.4 第二種模式)
...
- 互動結束(SSE 連線關閉)5 分鐘後 GC,呼應 idea_已過時.md §六「快取資料刪除,原始資料永遠留存於 Arweave」
...
> idea_已過時.md §3.5 原文提到的 Phantom 是 Solana 系錢包,EVM 鏈不適用,prototype 不納入。
...
對應 idea_已過時.md §九,以下功能在 prototype 完成後可加掛而不破壞既有架構:
